...
def main():
    ...
    ax.set(xlabel = 'User Age groups', title = 'User composition by age groups and gender')
    ...
    ax.set(xlabel = 'User Occupations', title = 'User composition by age and occupation')
    ...
    ax.set(title = 'Genres of movies in datatset', ylabel = '')
    ...
    ax.set_title('Co-occuring genres in movies')
    fig = ax.get_figure()
    fig.subplots_adjust(bottom=0.2)
    figures.append(fig)
    ...
    ax.set(title ='Frequencies of rating levels', xlabel = 'Rating Levels', ylabel = '#')
    ax.legend(title = 'Gender')
    fig = ax.get_figure()
    figures.append(fig)
    ...
    ax.set(title = 'Most rated movies (popular movies)', xlabel = 'Movies', ylabel = '# Ratings')
    ...
    ax.set_ylim([4.4,4.6])
    ax.xaxis.set_tick_params(labelsize=6)#, rotation = 30)
    ax.set(title = 'Best rated movies (Avg ratings for movies with more than 500 ratings)', xlabel = 'Movies', ylabel = 'Avg Ratings')
    fig = ax.get_figure()
    fig.subplots_adjust(bottom=0.5)
    figures.append(fig)
    ...
    ax.set(title= 'Number of ratings received by genres', xlabel = 'Genres', ylabel = '# Ratings')
    fig = ax.get_figure()
    fig.subplots_adjust(bottom=0.5)
    figures.append(fig)
    
    # Fig8: Show average ratings received by genre
    f, ax = plt.subplots()
    ...
    ax.set(title = 'Average ratings received for genres', xlabel = 'Genres', ylabel = 'Avg rating')
    ax.set_ylim([2,5])
    fig = ax.get_figure()
    fig.subplots_adjust(bottom=0.2)
    figures.append(fig)
    
    # Fig8: Show number unique users that have given rating per year
    f, ax = plt.subplots()
    unique_users = full_df.groupby(['timestamp', 'u_gender'])['u_id'].nunique()
    ax = unique_users.unstack().plot(kind = 'bar')
    ax.set(title = 'Unique active users (who have given rating) per year', xlabel = 'Year', ylabel = '# Active users')
    fig = ax.get_figure()
    figures.append(fig)
    
    # Fig9: Show average rating received by genre each year
    f, ax = plt.subplots()
    unique_users = full_df_explode.groupby(['timestamp', 'm_genres'])['rating'].mean()
    ax = unique_users.unstack().plot(kind = 'bar')
    ax.set(title = 'Average ratings for genres per year', xlabel = 'Year', ylabel = 'Average rating')
    ax.set_ylim([2,5])
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    fig = ax.get_figure()
